...
def dock(
    ...
    if not bb_residue and not bb_radius_ang:
        ...
        center, box_size = get_boundingbox_around_residue(
            str(receptor_pdb_fpath), bb_residue, bb_radius_ang
        )
    else:
        raise KeyError(
            f"If providing a bounding box residue, must provide residue name (like W23) and radius in angstroms."
        )
...
def get_boundingbox_around_residue(receptor_pdb_fpath, residue_id, radius_ang):
    parser = PDBParser()
    ...
    if Polypeptide.three_to_one(res.resname) is not res_type:
        raise KeyError(
            f"Residue {res_number} in the PDB is type {res.resname}, expected {res_type}"
        )
...
def get_energies(ligand_list, protein_type, protein_fpath, pdbqt_fpath):
...
def make_barplot(ligand_fnames, energieslist):
    ...
    data = pd.DataFrame({"fname": ligand_fnames, "energy": energieslist})

    data["ligand_name"] = data.fname.str.slice(stop=-6)
    ax = sns.barplot(data=data, x="ligand_name", y="energy").set(
...
def energies_list_for_protein(receptor_fpath):
    ...
    for l in ligands_fnames:
        energies.append(
            calc_energy(receptor_fpath, "Ligands_poses/" + r_fname + "/" + l)
        )
