...
VALID_AMINO_ACIDS = ['A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'Y']
...
def start_stage(fold_id: int, stage, email_on_completion):
  ...
  if stage == 'features':
    ...
    email_dependent_jobs = [features_job, models_job, decompress_pkls_job, annotate_job]
...
def back_translate(aa_seq):
  ...
  return biotools.reverse_translate(
    aa_without_u,
    table='Bacterial'
  )
...
class LocalStorageManager(StorageManager):
  ...
  def list_files(self, fold_id):
    ...
    return [
      ...
      for file in dir.glob('**/*') if not file.is_dir()
    ]
  ...
  def get_binary(self, fold_id, file_path):
    ...
    print(f'Download to server took {time.time() - download_start} seconds for {fpath}.', flush=True)
    return blob_bytes
...
class GcloudStorageManager(StorageManager):
    ...
    return [
      {
        ...
        'size': blob.size,
        'modified': (blob.updated - datetime.datetime.fromtimestamp(0, tz=timezone.utc)).total_seconds() * 1000.0,
      } for blob in blobs
    ]
  ...
  def get_binary(self, fold_id, relative_path):
    ...
    print(f'Download to server took {time.time() - download_start} seconds for {gcloud_path}.', flush=True)
    return blob_bytes
...
class FoldStorageUtil:
  ...
  def get_folds_with_state(
    ...
    query = db.session.query(Fold).options(joinedload(Fold.jobs), joinedload(Fold.docks)).join(Fold.user)
  ...
  def validate_sequence(self, fold_name, sequence, af2_model_preset):
    ...
    if ':' in sequence or ';' in sequence:
      if af2_model_preset != 'multimer':
        raise BadRequest(f'This sequence looks like a multimer. Multimers are only supported when using "AF2" and using the AF2 model preset "multimer" (see Advanced Options).')
      chains = []
    ...
    for chain_name, chain_seq in chains:
      ...
      for ii, aa in enumerate(chain_seq):
        if aa not in VALID_AMINO_ACIDS:
          raise BadRequest(f'Invalid amino acid "{aa}" at index {ii+1} in chain {chain_name}. Valid amino acids are {"".join(VALID_AMINO_ACIDS)}.')
  ...
  def make_new_folds(
    ...
    try:
      ...
      for fold_data in folds_data:
        ...
        self.validate_sequence(fold_data['name'], fold_data['sequence'], af2_model_preset)

        existing_entry = db.session.query(Fold).filter(Fold.name == fold_data['name']).first()
        if existing_entry:
          if not skip_duplicate_entries:
            raise BadRequest(f'Someone has already submitted a fold named {fold_data["name"]} ({existing_entry.id}).')

          if existing_entry.sequence == fold_data['sequence']:
            # Sweet, so the entry already exists, we can just skip it.
            continue
          else:
            raise BadRequest(f'Fold {fold_data["name"]} already exists ({existing_entry.id}) and its sequence does not match this new request.')
    ...
    for model in new_fold_models:
      try:
        ...
        print(f'Bad news, a gcloud write failed for model {model.id}. ' +
  ...
  def get_fold_pdb(self, fold_id, ranked_model_number):
    return self.storage_manager.get_binary(fold_id, f'ranked_{ranked_model_number}.pdb').decode()
  ...
  def get_model_pae(self, fold_id, model_number):
    bytes_str = self.storage_manager.get_binary(fold_id, f'ranked_{model_number}/predicted_aligned_error.npy')
    try:
  ...
  def get_contact_prob(self, fold_id, model_number, dist_thresh=12):
    bytes_str = self.storage_manager.get_binary(fold_id, f'ranked_{model_number}/contact_prob_{dist_thresh}A.npy')
    try:
      return np.load(io.BytesIO(bytes_str), allow_pickle=True)
    except Exception as e:
      print(e, flush=True)
      raise BadRequest(f'Failed to unpack file contact prob for {fold_id} model {model_number} threshold {dist_thresh}A ({e}).')


  def get_antismash(self, fold_id):
    bytes_str = self.storage_manager.get_binary(fold_id, f'antismash/parsed/simple.json')
    try:
