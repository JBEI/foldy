...
simple_invokation_fields = ns.model('SimpleInvokationFields', {
  'id': fields.Integer(required=True),
  'type': fields.String(required=False),
  'state': fields.String(required=False),
})

full_invokation_fields = ns.clone('InvokationFields', simple_invokation_fields, {
  'job_id': fields.String(required=False),
  'log': fields.String(required=False),
  'timedelta_sec': fields.Float(readonly=True, required=False, attribute=lambda r: r.timedelta.total_seconds() if r.timedelta else None),
})


dock_fields = ns.model('DockFields', {
  ...
  'bounding_box_residue': fields.String(required=False, nullable=True, help="Residue to center bounding box on, like Y74."),
  'bounding_box_radius_angstrom': fields.Float(required=False, nullable=True, help='Radius of bounding box in angstroms.'),

  'invokation_id': fields.Integer(required=False),
  'pose_energy': fields.Float(required=False),
})

get_folds_fields = ns.model('GetFolds', {
  'filter': fields.String(required=False)
})

fold_fields = ns.model('Fold', {
...
new_folds_fields = ns.model('NewFolds', {
...
fold_pdb_fields = ns.model('FoldPdb', {
...
class FoldResource(Resource):
  ...
  def get(self, fold_id, model_number):
    ...
    json_resp = (
      '{ "pae": ' +
      convert_array_to_json_string(pae) +
      ' }'
    )
    resp = make_response(
      json_resp,
...
contact_prob_fields = ns.model('ContactProb', {
...
class ContactProbResource(Resource):
  ...
  def get(self, fold_id, model_number):
    ...
    json_resp = (
      '{ "contact_prob": ' +
      convert_array_to_json_string(contact_prob) +
      ' }'
    )
...
class DockResource(Resource):
  ...
  def post(self):
    ...
    new_invokation_id = get_job_type_replacement(
      fold,
      f'dock_{ligand_name}'
    )
...
queue_test_job_fields = ns.model('QueueTestJobField', {
  'queue': fields.String(),
...
class SendTestEmailResource(Resource):
  ...
  def post(self):
    ...
    q.enqueue(
      jobs.send_email,
      1, 'test_prot', 'jacoberts@gmail.com',
      job_timeout='6h',
      result_ttl=48*60*60,  # 2 days
    )
    return True
...
queue_job_fields = ns.model('QueueFold', {
...
stamp_dbs_fields = ns.model('StampDbsFields', {
...
remove_failed_jobs_fields = ns.model('RemoveFailedJobs', {
  'queue': fields.String()
})
...
kill_worker_fields = ns.model('KillWorkerFields', {
...
class SetAllUnsetModelPresets(Resource):
  def post(self, folds_range):
    ...
    folds_in_range = db.session.query(Fold.id).filter(and_(Fold.id >= fold_lower_bound, Fold.id < fold_upper_bound))
