...
export const downloadFileStraightToFilesystemFancy = async (
    foldId: number,
    filePath: string,
    newFileName?: string
): Promise<void> => {
    console.log(`Downloading ${foldId} ${filePath} using native browser download`);

    // Create a direct link to the backend endpoint
    // Assuming cookie-based authentication is handled automatically
    let downloadUrl = `${import.meta.env.VITE_BACKEND_URL}/api/fancyfile/download/${foldId}/${filePath}`;

    // Add filename query parameter if provided
    if (newFileName) {
    ...
    if (newFileName) {
        ...
        console.log(`Setting link.download to newFileName:`, newFileName);
    } else {
        // Extract filename from path
        const pathParts = filePath.split('/');
        link.download = pathParts[pathParts.length - 1];
        console.log(`No newFileName provided, extracted from path:`, link.download);
    }

    console.log(`Final link.download value:`, link.download);

    // Add to DOM temporarily (required for Firefox)
    ...
    console.log('Download initiated via browser native mechanism');
};
